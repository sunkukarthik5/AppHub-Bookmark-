...
function renderCategoryDropdown() {
  ...
  // Set default value
  if (appCategory.options.length > 0) {
    appCategory.value = appCategory.options[0].value;
  }
}
    ...
    iconOption.addEventListener('click', () => {
      ...
      if (selectedCategoryId) {
        ...
        if (categoryIndex !== -1) {
          ...
          // Update in user categories if it's a user-added category
          const userCategoryIndex = userCategories.findIndex(cat => cat.id === selectedCategoryId);
          if (userCategoryIndex !== -1) {
            userCategories[userCategoryIndex].icon = icon;
            saveUserCategories();
          }
...
function closeCategoryContextMenu(e) {
...
function closeAppContextMenu(e) {
...
function saveRename() {
  ...
  if (newName && selectedCategoryId) {
    ...
    if (categoryIndex !== -1) {
      ...
      const userCategoryIndex = userCategories.findIndex(cat => cat.id === selectedCategoryId);
      if (userCategoryIndex !== -1) {
        userCategories[userCategoryIndex].name = newName;
        saveUserCategories();
      }
...
function logout() {
  if (confirm('Are you sure you want to logout?')) {
    // Clear user data
    localStorage.removeItem('apphub-user-profile');
    localStorage.removeItem('apphub-theme');
...
function setupEventListeners() {
  ...
  document.addEventListener('click', (e) => {
    if (!e.target.closest('.category-menu') && !e.target.closest('.app-menu')) {
      if (!categoryContextMenu.contains(e.target)) {
        categoryContextMenu.classList.remove('show');
      }
      if (!appContextMenu.contains(e.target)) {
